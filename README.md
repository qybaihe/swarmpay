...
- → 通过 **Injective 链上真实交易**(当前 direct `MsgSend`;CosmWasm 合约待部署)把预算按权重分发给各 agent 钱包
...
| 链上真实转账 `MsgSend` | ✅ | [tx 9477EC31](https://testnet.explorer.injective.network/transaction/9477EC31F870FFD079D65B0D863174DBBF281445F74CACB16AB21C6215416077) 0.1 INJ 上链,证明签名广播能力 |
| 全真链分润 `/api/injective/run` | ✅ | direct 模式:蜂群答案 + 多笔链上分润交易,4 个 agent + treasurer 按权重真实收到 INJ |
| CosmWasm 分润合约 | ✅ | 16 单测过、wasm 编译成功(302K),代码就绪待部署 |
...
POST /api/injective/run                  ← src/injective/routes.ts
   ...
   4. split-executor 链上分润              ← split-executor.ts
      ...
      · contract 模式:CosmWasm Distribute(代码就绪,部署后启用)
   5. 返回 答案 + trace + 链上交易回执
```

**双入口**:`/v1/chat/completions` 保持 OpenAI 兼容调用体验,结算已切到链上 INJ;`/api/injective/*` 提供链上蜂群 demo、自检、余额与流水接口,由 `INJECTIVE_NETWORK=mock|testnet` 切换,默认 mock 永不黑屏。
...
docs/injective-plan/        15 份完整改造/提交方案文档
```
...
|---|---|
| 创新性 | 蜂群协作 + 链上分润,"越用越聪明 × 越用越值钱",面向 Injective 生态的 agent 分润与自主支付基础设施 |
| 技术实现 | 集成 Injective testnet + sdk-ts + direct `MsgSend` 分润;CosmWasm 分润合约代码/测试/wasm 就绪;AI(蜂群)与链上(分润)深度结合,全真链验证 |
| 应用价值 | 解决 agent 协作的价值分配黑箱,可延伸到 agent 市场/众包/外包结算 |
| 产品体验 | Vue Flow 协作可视化 + 分润流向图 + 交易 timeline,agent 协作与资金流向都可视 |
| 生态契合 | 复用 Injective sdk-ts / CosmWasm / INJ；官方 iAgent SDK / Injective Agent SDK / MCP Server 作为路线图对齐项，当前尚未集成 |

---

## 📚 文档

完整设计与改造方案见 [`docs/injective-plan/`](docs/injective-plan/README.md)(00-14 共 15 份):总纲、架构、链层、合约、payer agent、API 契约、前端、部署、Demo 脚本、Pitch Deck 大纲、报名表单文案、后续打磨规划与 Nova 提交核对清单。

## 🧾 Nova 提交状态

官方要求对照见 [`docs/injective-plan/14-NOVA-SUBMISSION-CHECKLIST.md`](docs/injective-plan/14-NOVA-SUBMISSION-CHECKLIST.md)。当前代码、链上 testnet 集成、README 与 Pitch Deck 已基本满足提交水位；提交前仍需补齐 Demo 视频链接、团队真实信息，并将本地改动 commit/push 到公开 GitHub 仓库。
