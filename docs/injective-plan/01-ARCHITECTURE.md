...
当前实现保留 OpenAI 兼容的 `/v1/chat/completions` 调用体验，同时新增 `/api/injective/*` 链上 demo、自检、余额与流水接口。旧 credits 记账已经迁移为链上 INJ 结算；蜂群编排内核仍由 `runSwarm()` 复用。

```
                     ┌──────────────────────────┐
   OpenAI 兼容客户端 ──→│ POST /v1/chat/completions │──→ 链上 INJ 计费 + 分润
                     └──────────────────────────┘
   ...
   (连 Keplr)        │  (新路由)                │
        ...
        (chain.getBalance) (原内核复用)     (split-executor)
                                              │
                                   ┌──────────┴──────────┐
                                   ▼                     ▼
                             direct MsgSend        payer agent
                             (当前已跑通)          (动态调权)
                             CosmWasm 合约
                             (代码就绪待部署)
```
...
3. routes.ts:
   a. chain.getBalance(senderAddr) → 余额 < 预算？ 402
   b. runSwarm({ tier, messages }) → { content, trace }   ← 原蜂群内核复用
   c. splitExecutor.distribute(trace, budgetAmount, denom):
      - 按 trace.rewardSplit 计算各 archetype 应得份额
      - payer agent 决策：当前 direct 多笔 `MsgSend`;合约部署后可切 `MsgExecuteContract`
